...
| D19 | Docker 内权限策略 | **对内 crew 全放开**：main、content-producer、it-engineer 的 `crew-type: internal` → `security: full`。**对外 crew**：`crew-type: external` → 默认 `deny`；sales-cs 用 `ALLOWED_COMMANDS` `+` 条目在 deny 上凿洞成 allowlist（只放行客服脚本）。理由：Docker 已隔离宿主 OS，内 crew 放开消除 exec allowlist miss 摩擦（复合命令 `&&`、`cd`/`bash` 前缀、相对路径误拼，见 `docs/feedback/2026-04-18-exec-allowlist-compound-command.md`）；sales-cs 面对未受信外部输入，allowlist 是 prompt injection 防线（防外发凭据/乱发客户消息），与容器隔离正交，必须保留。**2026-07-07 简化**：删 `command-tier`（T0~T3 四档抽象）字段，权限改由 `crew-type` + `ALLOWED_COMMANDS` 两源决定（T1/T2 死代码清除）。落实点：Phase 7 crew 重构写 client 仓 SOUL.md 时 |
