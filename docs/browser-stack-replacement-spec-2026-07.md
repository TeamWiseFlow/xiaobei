...
3. ✅ **一次性 patch extension**（`browser-camoufox-pivot/patches/`，35 个单文件 patch）：3a 增量接线（`BROWSER_TARGETS` 加 `camoufox`、`browser-tool.ts` 加 camoufox 早返回调 adapter、**default 改成 camoufox**——无 target 无 node 无 existing-session profile 时走 camoufox，3b 删了 local-managed 故旧默认路径已死；upload 路径校验前置到 camoufox 早返回前闭合 camoufox+upload 绕过 `resolveExistingUploadPaths` 的安全缺口）+ 3b 全量减法（删 `sandbox` target + 10 文件 + local-managed 分支 + sandbox 桥 + plugin-sdk/browser-bridge facade + `--browser` CLI flag + `/sandbox/novnc` route）。原 001 monolith 已按「一个 patch 只改一个上游文件」拆成 35 个单文件 patch（降低上游漂移失效面），干净上游逐个 `git apply --3way` 验证通过，tsgo 无新错，`browser-tool.test.ts` 70 测试过（测 host 行为的 case 显式 `target: "host"`）。线 2（host/node + existing-session/remote-cdp + loopback host bridge + `browserAllowHostControl`）保留。
