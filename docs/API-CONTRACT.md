...
**不提供 proxy 端点**（不在 relay 上替 client 调小红书/抖音）：relay 是固定公网 IP，频繁替 client 代请求会被平台风控/封 IP。client 拿到签名 header 后，**在自己的浏览器上下文/小程序里发请求到平台 API**。
...
### wx-mp（公众号草稿发布，relay 导入 `@wenyan-md/core` 作为库）

| 方法 | 路径 | 入参 | 出参 |
|------|------|------|------|
| GET | `/wx-mp/health` | — | `{ success: true }`（仅 3004 直连，不公开） |
| POST | `/wx-mp/publish` | multipart：`markdown`(text) + `wechat_app_id`(text) + `wechat_app_secret`(text) + `theme`?(text) + `images`?(file，可多) | `{ success, data: { media_id?, article_url? } }` |

错误码：`MISSING_MARKDOWN` / `MISSING_APP_ID` / `MISSING_APP_SECRET`（400）；发布失败 502。仅支持文本+图片（无视频）。两种模式由核心按 `image_list` 是否为空自动分支（普通文章 / 小绿书）。
...
| POST | `/wxwork/drive/space-setting` | JSON：`{ corp_id, corp_secret, spaceid, share_url_no_approve?, share_url_no_approve_default_auth?, enable_watermark?, enable_confidential_mode?, default_file_scope?, ban_share_external? }` | `{ ok, detail }` |
...
| POST | `/wxwork/drive/upload-image` | multipart：`corp_id`+`corp_secret`+`spaceid`+`fatherid`+`file_name`?+`file`(image ≤10M) | `{ ok, fileid, detail }` |
| POST | `/wxwork/drive/upload-video` | multipart：`corp_id`+`corp_secret`+`spaceid`+`fatherid`+`file_name`?+`file`(video/大文件，relay 分块) | `{ ok, fileid, fast_forward }` |
| POST | `/wxwork/drive/list-files` | JSON：`{ corp_id, corp_secret, spaceid, fatherid, sort_type, start, limit }` | `{ ok, detail: { has_more, next_start, file_list:{item[]} } }` |
| POST | `/wxwork/drive/file-info` | JSON：`{ corp_id, corp_secret, fileid }` | `{ ok, detail: { file_info } }` |
...
错误码：`MISSING_CORP_CREDENTIALS`（400，缺 corp_id 或 corp_secret）；`MISSING_FIELD`（400，drive 管理接口缺必填字段）；`GETTOKEN_FAILED`（502，corp_secret 错或 corp_id 不存在）；上游 `errcode != 0`（400，`detail` 里是原始 errcode/errmsg）。`corp_id`/`corp_secret` 在转发企业微信前由 relay 剥离，不下发。tokenCache 按 `(corp_id, corp_secret)` 分桶。

> 微盘硬约束：**应用只能往自己创建的文件夹里传文件**，故上传前必须先 `create-folder` 拿 `fatherid`。详见 [WXWORK-DRIVE-API.md](./WXWORK-DRIVE-API.md)。
