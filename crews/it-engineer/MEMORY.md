...
- 避免小白用户运行期 pip（小白场景是 Docker 部署态；本机源码部署场景下用户已有 Python 环境，但镜像预装策略仍适用）
...
- **软链方案**：本机开发实例可软链 `~/.openclaw/skills/<name>` → `<repo>/skills/<name>`，改 repo 即时生效；Docker 镜像维持 COPY（重建即更新）。

### login-manager 状态机

- **中央存储**：`~/.openclaw/logins/{platform}.json`（camoufox-cli 原生 JSON 格式 = Playwright `add_cookies` 期望格式）。
- **平台常量**：`douyin` / `bilibili` / `kuaishou` / `xhs-publish` / `xhs-browse` / `weibo` / `zhihu` / `wechat-channels` / `wx-mp`。
- **9 个子命令**（兼容 + 新增）：
  - 兼容（维持原 CLI 语义）：`check` / `read` / `write` / `status-all`
  - 新增：`qr-headless` / `qr-confirm` / `cookie-export` / `cookie-import` / `session-cleanup`
...
- **当前路径**：`<WISEFLOW_PROJECT_ROOT>/awada/`，**单层**结构（非 `awada/awada-extension/`）。通过 ioredis 直连 Redis。
- **目标路径**：改 HTTP/WS transport + `X-OFB-Key` header：
...
- **本轮新仓**：`~/wiseflow`（client 仓，master 分支）。
- **部署策略**：**先源码部署本机**（不走 Docker；拉新仓代码 + copy openclaw + apply skill 替换 + 重启 Gateway），验证通过后再做 Dockerfile。
...
## 关于 xiaobei 项目(我正在维护的项目)

项目背景、功能介绍和目录结构详见工作区中的**项目背景.md**(由部署脚本自动同步,每次升级均为最新版)。

### 项目基本信息
- **项目名称**:xiaobei（main agent，对外品牌名；原名 wiseflow）
...
- **历史曾用名**:openclaw_for_business、OFB、wiseflow(代码已合并,统一使用 xiaobei)
- **产品拆分后（2026-07）**：
  - 客户端仓 = `~/wiseflow`（master 分支）
...
> 实际 xiaobei 项目路径记录在 `OFB_ENV.md`(同目录,历史命名保留),每次运行 `setup-crew.sh` 自动更新。
...
> awada 单层结构，路径 = `<WISEFLOW_PROJECT_ROOT>/awada/`（非 `awada/awada-extension/`）。
