...
> **严禁**：**严禁 camoufox-cli（浏览器方案）通过 `cookies import` 导入 cookie 造一个登录会话**。浏览器操作一律走 login-manager 真实登录后的**持久化 session**（登录态 + 指纹冻结在 session profile 里），不开临时 session 再 import cookie 那一套。xhs `a1`/`websectiga` 等设备指纹 cookie 导入到不同指纹的浏览器会话会错配 → 被风控检测（2026-06-29 CDP 注入 22 cookie 触发风控的教训）。
>
